# -*- coding: utf-8 -*-
"""
Created on Thu Mar 18 21:39:15 2021
...
def watchlist_reader():
    ...
    while (i < len(tickers)): #loops through every ticker in the list that was selected
        try:
            ...
            current = r.stocks.get_stock_historicals(stock,interval='day',span='3month',bounds='regular',info = None)
            close_prices = []
            j = 0
            while j < len(current):
                close_prices.append(float(current[j]['close_price']))
                j+=1
            period = 20 # Set period length used in the calculations
            moving_avg_calc(close_prices,period)
